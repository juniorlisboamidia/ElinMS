...
// Free an ONLINE player from jail, live. Proxies to the Admin API.
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${GAME_API_URL}/unjail`, {
      method: "POST",
    ...
    return NextResponse.json({ error: "Failed to unjail. Is the game server running?", details: err.message }, { status: 500 });
  }
}
