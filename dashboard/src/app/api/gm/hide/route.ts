...
// Make an ONLINE player invisible to others (GM hide), live. Proxies to the Admin API.
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${GAME_API_URL}/hide`, {
      method: "POST",
    ...
    return NextResponse.json({ error: "Failed to hide. Is the game server running?", details: err.message }, { status: 500 });
  }
}
