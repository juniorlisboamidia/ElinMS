...
// Disconnect an ONLINE player from the server, live. Proxies to the Admin API.
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${GAME_API_URL}/kick`, {
      method: "POST",
    ...
    return NextResponse.json(
      { error: "Failed to kick. Is the game server running?", details: err.message },
      { status: 500 },
    );
  }
}
